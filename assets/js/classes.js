...
//Таск: Реалізувати клас для телефонів (марка, модель, колір, ціна, рік випуску)
// реалізувати метод для розрахунку віку телефону
// створити екземпляр класу, викликати до нього метод
...
class Phone {
    ...
    getPhoneAge() {
        ...
        // тут я використала вбудований обєкт Date, для відображення пооточної дати і викликала для неї функцію getFullYear() для отримання 4 цифр року
        // бачимо що Date використовується лише як функція-конструктор.
        // Варіант перший через змінні створити дату, потім окремо витягти рік методом getFullYear().
        // const currentDate = new Date();
        // const currentYear = currentDate.getFullYear();

        // Варіант другий вписати коротко одразу в шаблонному літералі

        return `This phone is ${new Date().getFullYear() - this.year} years old :)`
    }
}
...
// Ще одна задача на два класи які будуть пересікатись в розрахунку

// Задача:
// Створити клас авто
// Створити клас паливо
// порахувати загальну вагу автомобіля (вага авто + вага палива(обєм * щільність))
...
class Vehicle {
    ...
    //метод який обчислює повну вагу авто: власна вага ownWeght + вага палива
    getFullWeight() {
        return this.ownWeight + this.fuel.getWeight();
    }
}
                                    // це посилання на попередній обєкт. тобто тепер обєкт BMW містить в собі ще обєкт benzin (обєкт в обєкті)
const BMW = new Vehicle('BMW', 4000, benzin);



// ------------------------------------------------------------------------------------------------------------------

        // Вбудований об'єкт Date (Date object)


// Об'єкт Date у JavaScript — це вбудований об'єкт, який дозволяє працювати з датами та часом.

// new Date()  - отримати поточну дату та час
// new Date().getDate() - отримати з поточної дати тільки число дня
// new Date().getFullYear() - отримати з поточної дати лише рік



// Основні моменти:
// Призначення: Дозволяє створювати, читати, змінювати та форматувати дати і час.

// Створення:
// new Date(): Створює об'єкт, що представляє поточну дату і час.
// new Date(рік, місяць, день, год, хв, сек, мс): Створює об'єкт для конкретної дати (місяці від 0 до 11!).
// new Date(мілісекунди): Створює дату на основі мілісекунд з 1 січня 1970 року (UTC).
// new Date("рядок дати"): Створює дату з рядка (наприклад, "2023-01-15" або "Jan 15, 2023").

// Отримання компонентів дати/часу (геттери):
// getFullYear(): Рік (4 цифри).
// getMonth(): Місяць (0-11, де 0 = січень).
// getDate(): День місяця (1-31).
// getDay(): День тижня (0-6, де 0 = неділя).
// getHours(), getMinutes(), getSeconds(), getMilliseconds(): Компоненти часу.
// Всі ці методи мають аналоги getUTC...() для універсального координованого часу.

// Встановлення компонентів дати/часу (сеттери):
// setFullYear(), setMonth(), setDate(), setHours() тощо.

// Перетворення в рядок:
// toString(): Повертає дату як рядок.
// toDateString(), toTimeString(), toISOString(): Специфічні формати рядків.

// Важливо: Об'єкти Date представляють моменти часу в мілісекундах з 1 січня 1970 року, 00:00:00 UTC (епоха Unix).